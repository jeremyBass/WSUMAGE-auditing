<?php
class Wsu_Logger_Model_Stock_Observer {
    public function addStockMovementsTab() {
        ...
        if ($block) {
            $block->addTab('stock_movements', array(
                'after' => 'inventory',
                'label' => Mage::helper('wsu_logger')->__('Stock Movements'),
                'content' => $layout->createBlock('wsu_logger/adminhtml_stock_movement_grid')->toHtml()
            ));
        }
    }
    ...
    public function catalogProductImportFinishBefore($observer) {
        ...
        $resource   = Mage::getResourceModel('wsu_logger/stock_movement');
