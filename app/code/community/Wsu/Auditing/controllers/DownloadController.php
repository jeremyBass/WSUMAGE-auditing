<?php
require_once '../../Logger/controllers/Mage/Downloadable/controllers/DownloadController.php';
class Wsu_Auditing_DownloadController extends Mage_Downloadable_DownloadController {
    public function linkAction() {
        ...
        if (!Mage::helper('downloadable')->getIsShareable($linkPurchasedItem)) {
            ...
            if (!$customerId) {
                ...
                $this->_getCustomerSession()->setBeforeAuthUrl(Mage::getUrl('downloadable/customer/products/'), array(
                    '_secure' => true
                ));
                return;
            }
        ...
        if ($status == Mage_Downloadable_Model_Link_Purchased_Item::LINK_STATUS_AVAILABLE && ($downloadsLeft || $linkPurchasedItem->getNumberOfDownloadsBought() == 0)) {
            ...
            try {
                ...
                Mage::helper('wsu_auditing')->log($linkPurchasedItem->getLinkTitle(), $linkPurchasedItem->getLinkFile(), $linkPurchasedItem->getProductId(), $this->_getCustomerSession()->getCustomerId());
